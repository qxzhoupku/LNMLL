...
x=np.linspace(-pi,pi-pi*2/1023,1024)
total_loss=omega_p/omega_m/Q_totp*2*pi
D=-0.5*beta2*L_d
...
# 初始signal光场为噪声
A=np.array([random.random()*np.exp(1.0j*random.random()*2*pi) for i in range(1024)])*1e-3
